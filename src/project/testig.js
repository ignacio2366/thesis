...
function Writer() {
  ...
  }, [headline, categories, story, file, cite]);
  ...
  const getDraftSources = async () => {
    ...
    var draftSources = await WriterModule.getDraftedNews(cite);
    var response = JSON.parse(draftSources);

    setHeadline(response[0].headline);
    setStory(response[0].content);
    setstoryTag(response[0].contenttag);
    setImage(response[0].image.replace("C:/xampp/htdocs", process.env.REACT_APP_PHP_URL));
